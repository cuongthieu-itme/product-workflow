...
export const AddMaterialDialog = () => {
  const { watch } = useFormContext<CreateProductInputType>();
  ...
  const { control } = useFormContext<CreateProductInputType>();
  ...
  const materialList =
    materials?.data.filter(
      (material) =>
        !watch("productMaterials").some(
          (item) => item.materialId === material.id
        ) && material.name.toLowerCase().includes(debouncedSearch.toLowerCase())
    ) || [];
