...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json({ success: false, message: "Failed to fetch steps" }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!stepId || !status) {
      return NextResponse.json({ success: false, message: "Step ID and status are required" }, { status: 400 })
    }
    ...
    if (status === "completed" && productData.currentStep === stepSnap.data().order) {
      const stepStatuses = ["draft", "review", "design", "production", "marketing", "launch", "completed"]
    ...
    return NextResponse.json({ success: false, message: "Failed to update step" }, { status: 500 })
  }
}
