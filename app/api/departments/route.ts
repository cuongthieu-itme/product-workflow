...
export async function POST(request: NextRequest) {
  ...
  if (session.user.role !== "admin") {
    return NextResponse.json({ success: false, message: "Admin permission required" }, { status: 403 })
  }
  ...
  try {
    ...
    if (departments.some((dept) => dept.id === body.id)) {
      return NextResponse.json({ success: false, message: "Department ID already exists" }, { status: 400 })
    }
