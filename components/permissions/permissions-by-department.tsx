          ...
          <CardContent className="pt-6">
            <div className="space-y-6">
              {workflowAreas.map((area) => {
                const currentPermission = permissions[selectedDepartment]?.[area.id] || "hide"
                ...
                return (
                  <div key={area.id} className="space-y-2">
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) =>
                        onUpdatePermission(selectedDepartment, area.id, value as PermissionType)
                      }
                      className="flex flex-col space-y-1"
                    >
                      {permissionTypes.map((permission) => (
                        <div key={permission.id} className="flex items-center space-x-2">
                          <RadioGroupItem value={permission.id} id={`${area.id}-${permission.id}`} />
                          <Label htmlFor={`${area.id}-${permission.id}`} className="flex flex-col">
