          ...
          {campaigns.map((campaign) => {
            ...
            return (
              <TableRow key={campaign.id}>
                <TableCell className="font-medium">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground">
                      Kênh: {campaign.channels.map((c) => channelConfig[c]).join(", ")}
                    </span>
                  </div>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <div className="flex flex-col">
                    <span className="text-xs text-muted-foreground">Bắt đầu: {formatDate(campaign.startDate)}</span>
                    <span className="text-xs text-muted-foreground">Kết thúc: {formatDate(campaign.endDate)}</span>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground">
                      Đã chi: {formatCurrency(campaign.spent)} ({Math.round((campaign.spent / campaign.budget) * 100)}%)
                    </span>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs text-muted-foreground">{campaign.progress}% hoàn thành</span>
                  </div>
                </TableCell>
