...
interface WorkflowProcessContextType {
  ...
  deleteStepField: (
    ...
    stepId: string,
    fieldId: string
  ) => boolean // Trả về false nếu không thể xóa
...
export function WorkflowProcessProvider({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        if (storedProcesses) {
          ...
          localStorage.setItem(
            'workflowProcesses',
            JSON.stringify(defaultProcesses)
          )
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !isFirstMount.current) {
      try {
        localStorage.setItem(
          'workflowProcesses',
          JSON.stringify(workflowProcesses)
        )
      } catch (error) {
  ...
  const addWorkflowStep = (
    ...
    const newStep: WorkflowStep = {
      ...
      id: stepId,
      order:
        workflowProcesses.find((p) => p.id === processId)?.steps.length || 0,
      fields: [
  ...
  const deleteWorkflowStep = (processId: string, stepId: string) => {
    ...
    setWorkflowProcesses((prev) =>
      prev.map((process) => {
        if (process.id === processId) {
          const filteredSteps = process.steps.filter(
            (step) => step.id !== stepId
          )
