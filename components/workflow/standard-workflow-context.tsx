...
export interface StepField {
  id: string
  name: string
  type: "text" | "date" | "select" | "user" | "checkbox" | "number" | "currency" | "variable"
  required: boolean
  description?: string
...
interface StandardWorkflowContextType {
  ...
  addStandardWorkflowStep: (step: Omit<StandardWorkflowStep, "id" | "order">, userId?: string) => string
  ...
  reorderStandardWorkflowSteps: (steps: StandardWorkflowStep[], userId?: string) => void

  // Quản lý trường trong bước
  addStepField: (stepId: string, field: Omit<StepField, "id">, userId?: string) => string
  updateStepField: (stepId: string, fieldId: string, updates: Partial<Omit<StepField, "id">>, userId?: string) => void
  ...
  // Quản lý biến có sẵn
  addAvailableVariable: (variable: Omit<AvailableVariable, "id">, userId?: string) => string
  updateAvailableVariable: (id: string, updates: Partial<Omit<AvailableVariable, "id">>, userId?: string) => void
...
export function StandardWorkflowProvider({ children }: { children: ReactNode }) {
    ...
    if (!isInitialized.current) {
      ...
      try {
        ...
        if (savedVariables) {
          ...
          setAvailableVariables(DEFAULT_AVAILABLE_VARIABLES)
          localStorage.setItem("availableVariables", JSON.stringify(DEFAULT_AVAILABLE_VARIABLES))
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (isInitialized.current) {
      localStorage.setItem("workflowChangeHistory", JSON.stringify(changeHistory))
    }
  }, [changeHistory])

  useEffect(() => {
    if (isInitialized.current) {
      localStorage.setItem("availableVariables", JSON.stringify(availableVariables))
    }
  }, [availableVariables])
  ...
  const initializeStandardWorkflow = () => {
    ...
    localStorage.setItem("standardWorkflow", JSON.stringify(newStandardWorkflow))
  ...
  const updateStandardWorkflow = (
    ...
    if (changes.length > 0) {
      addChangeHistory("update", "workflow", updatedWorkflow.id, changes, userId)
    }
  ...
  const addStandardWorkflowStep = (step: Omit<StandardWorkflowStep, "id" | "order">, userId = "system") => {
    if (!standardWorkflow) return ""
    ...
    addChangeHistory("create", "step", stepId, [{ field: "step", newValue: newStep.name }], userId)
  ...
  const updateStandardWorkflowStep = (
    ...
    if (updates.description && updates.description !== oldStep.description) {
      changes.push({ field: "description", oldValue: oldStep.description, newValue: updates.description })
    }

    if (updates.estimatedDays !== undefined && updates.estimatedDays !== oldStep.estimatedDays) {
      changes.push({ field: "estimatedDays", oldValue: oldStep.estimatedDays, newValue: updates.estimatedDays })
    }

    if (updates.isRequired !== undefined && updates.isRequired !== oldStep.isRequired) {
      changes.push({ field: "isRequired", oldValue: oldStep.isRequired, newValue: updates.isRequired })
    }

    if (updates.notifyBeforeDeadline !== undefined && updates.notifyBeforeDeadline !== oldStep.notifyBeforeDeadline) {
      changes.push({
    ...
    if (updates.assigneeRole && updates.assigneeRole !== oldStep.assigneeRole) {
      changes.push({ field: "assigneeRole", oldValue: oldStep.assigneeRole, newValue: updates.assigneeRole })
    }

    if (updates.hasCost !== undefined && updates.hasCost !== oldStep.hasCost) {
      changes.push({ field: "hasCost", oldValue: oldStep.hasCost, newValue: updates.hasCost })
    }
  ...
  const deleteStandardWorkflowStep = (stepId: string, userId = "system") => {
    ...
    const isUsedInSubWorkflow = subWorkflows.some((sw) => sw.visibleSteps.includes(stepId))
    if (isUsedInSubWorkflow) {
      toast({
        title: "Không thể xóa",
        description: "Bước này đang được sử dụng trong một hoặc nhiều quy trình con.",
        variant: "destructive",
      })
      return false
    }
    ...
    addChangeHistory("delete", "step", stepId, [{ field: "step", oldValue: step.name, newValue: null }], userId)
  ...
  const reorderStandardWorkflowSteps = (steps: StandardWorkflowStep[], userId = "system") => {
    if (!standardWorkflow) return
  ...
  const addStepField = (stepId: string, field: Omit<StepField, "id">, userId = "system") => {
    if (!standardWorkflow) return ""
    ...
    addChangeHistory("create", "field", fieldId, [{ field: "field", newValue: newField.name }], userId)
  ...
  const updateStepField = (
    ...
    if (updates.description && updates.description !== oldField.description) {
      changes.push({ field: "description", oldValue: oldField.description, newValue: updates.description })
    }

    if (updates.type && updates.type !== oldField.type) {
      changes.push({ field: "type", oldValue: oldField.type, newValue: updates.type })
    }

    if (updates.required !== undefined && updates.required !== oldField.required) {
      changes.push({ field: "required", oldValue: oldField.required, newValue: updates.required })
    }
  ...
  const deleteStepField = (stepId: string, fieldId: string, userId = "system") => {
    if (!standardWorkflow) return false
    ...
    addChangeHistory("delete", "field", fieldId, [{ field: "field", oldValue: field.name, newValue: null }], userId)
  ...
  const createSubWorkflow = (
    ...
    addChangeHistory("create", "workflow", id, [{ field: "sub_workflow", newValue: newSubWorkflow.name }], userId)
  ...
  const updateSubWorkflow = (
    ...
    if (updates.name && updates.name !== oldSubWorkflow.name) {
      changes.push({ field: "name", oldValue: oldSubWorkflow.name, newValue: updates.name })
    }

    if (updates.description && updates.description !== oldSubWorkflow.description) {
      changes.push({ field: "description", oldValue: oldSubWorkflow.description, newValue: updates.description })
    }

    if (updates.statusId && updates.statusId !== oldSubWorkflow.statusId) {
      changes.push({ field: "statusId", oldValue: oldSubWorkflow.statusId, newValue: updates.statusId })
    }

    if (updates.visibleSteps && JSON.stringify(updates.visibleSteps) !== JSON.stringify(oldSubWorkflow.visibleSteps)) {
      changes.push({ field: "visibleSteps", oldValue: oldSubWorkflow.visibleSteps, newValue: updates.visibleSteps })
    }
  ...
  // Thêm biến có sẵn
  const addAvailableVariable = (variable: Omit<AvailableVariable, "id">, userId = "system") => {
    const id = generateId()
  ...
  const updateAvailableVariable = (id: string, updates: Partial<Omit<AvailableVariable, "id">>, userId = "system") => {
  ...
  const getChangeHistoryForEntity = (entityType: "workflow" | "step" | "field", entityId: string) => {
