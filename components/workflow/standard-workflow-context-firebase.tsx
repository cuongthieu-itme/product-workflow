...
export function StandardWorkflowProvider({
  ...
  const checkAndInitializeStandardWorkflows = async (): Promise<void> => {
    try {
      ...
      if (!standardWorkflowSnap.exists()) {
        console.log(
          '❌ Collection standardWorkflows không tồn tại, đang tạo...'
        )
        ...
        console.log(
          '✅ Standard workflow loaded successfully with',
  ...
  const deleteStandardWorkflowStep = async (
    stepId: string,
    userId = 'system'
  ): Promise<boolean> => {
  ...
  const reorderStandardWorkflowSteps = async (
    steps: StandardWorkflowStep[],
    userId = 'system'
  ): Promise<void> => {
    if (!standardWorkflow) return
  ...
  const addStepField = async (
    ...
    try {
      ...
      await addChangeHistory(
        ...
        'field',
        fieldId,
        [{ field: 'field', newValue: cleanField.name }],
        userId
      )
  ...
  const addAvailableVariable = async (
    variable: Omit<AvailableVariable, 'id'>,
    userId = 'system'
  ): Promise<string> => {
    // Implementation would go here
  ...
  const deleteAvailableVariable = async (
    id: string,
    userId = 'system'
  ): Promise<void> => {
    // Implementation would go here
