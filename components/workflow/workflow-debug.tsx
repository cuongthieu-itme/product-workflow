...
export function WorkflowDebug() {
  ...
  const [collections, setCollections] = useState<Record<string, { exists: boolean; count: number; error?: string }>>({})
  const [access, setAccess] = useState<{ read?: boolean; write?: boolean; error?: string }>({})
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Kiểm tra và khắc phục vấn đề kết nối với cơ sở dữ liệu quy trình</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <TabsContent value="status">
            {loading ? (
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-medium mb-2">Quyền truy cập Firebase</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center p-3 border rounded-md">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {access.read ? "Có thể đọc dữ liệu" : "Không thể đọc dữ liệu"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center p-3 border rounded-md">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {access.write ? "Có thể ghi dữ liệu" : "Không thể ghi dữ liệu"}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  <h3 className="text-lg font-medium mb-2">Trạng thái các collection</h3>
                      ...
                      <div key={name} className="flex items-center justify-between p-3 border rounded-md">
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {info.exists ? `${info.count} documents` : "Collection không tồn tại"}
                            </p>
                        ...
                        <div className="flex gap-2">
                          <Badge variant={info.exists ? "outline" : "secondary"}>{info.count} docs</Badge>
                          <Button
                            variant="ghost"
                            size="sm"
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium">
                  {selectedCollection ? `Dữ liệu trong ${selectedCollection}` : "Chọn collection để xem dữ liệu"}
                </h3>
                <div className="flex gap-2">
                  <select
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => selectedCollection && loadCollectionData(selectedCollection)}
              ...
              {loading ? (
                ...
                <div>
                  {collectionData[selectedCollection].length === 0 ? (
                    <div className="text-center p-6 border rounded-md">
                      <p className="text-muted-foreground">Không có dữ liệu trong collection này</p>
                    </div>
                  ) : (
                    <div className="border rounded-md overflow-auto max-h-[400px]">
                      <table className="w-full">
                        ...
                        <tbody>
                          {collectionData[selectedCollection].map((doc, index) => (
                            <tr key={doc.id} className={index % 2 === 0 ? "bg-muted/20" : ""}>
                              <td className="p-2 border-t font-mono text-sm">{doc.id}</td>
                ...
                <div className="text-center p-6 border rounded-md">
                  <p className="text-muted-foreground">Chọn một collection để xem dữ liệu</p>
                </div>
              )}
            </div>
          </TabsContent>
            ...
            <div className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Tạo các collection cần thiết cho quy trình nếu chúng chưa tồn tại</CardDescription>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-muted-foreground mb-4">
                    Hành động này sẽ tạo các collection sau nếu chúng chưa tồn tại:
                ...
                <CardFooter>
                  <Button onClick={initializeCollections} disabled={initializing}>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Kiểm tra lại trạng thái kết nối và các collection</CardDescription>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-muted-foreground">
                    Làm mới trạng thái để xem các thay đổi mới nhất trong cơ sở dữ liệu
                  </p>
                </CardContent>
                <CardFooter>
