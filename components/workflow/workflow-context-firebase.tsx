...
export function WorkflowProvider({ children }: { children: ReactNode }) {
  ...
  const [productStatuses, setProductStatuses] = useState<ProductStatus[]>(mockProductStatuses)
  ...
  const initializeFromOtherContexts = () => {
    try {
      ...
      if (standardWorkflow && standardWorkflow.steps && Array.isArray(standardWorkflow.steps)) {
        ...
        console.warn("StandardWorkflow không có hoặc không có steps, sử dụng quy trình mặc định")
      }

      // Thêm các quy trình con
      if (subWorkflows && Array.isArray(subWorkflows) && subWorkflows.length > 0) {
        subWorkflows.forEach((subWorkflow) => {
          ...
          convertedWorkflows.push({
            id: subWorkflow.id || `sub-${Math.random().toString(36).substr(2, 9)}`,
  ...
  const addWorkflow = async (workflow: Omit<Workflow, "id" | "createdAt" | "updatedAt">): Promise<Workflow> => {
  ...
  const isStatusHasWorkflow = (statusId: string, excludeWorkflowId?: string): boolean => {
