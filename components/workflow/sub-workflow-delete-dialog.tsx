...
export function SubWorkflowDeleteDialog({
  subWorkflow,
  trigger,
  onSuccess
}: SubWorkflowDeleteDialogProps) {
  ...
  const {
    deleteSubWorkflow,
    checkRequestsUsingSubWorkflow,
    migrateRequestsToStandardWorkflow
  } = useSubWorkflow()
  ...
  const handleOpenDialog = async () => {
    ...
    try {
      ...
      const { count, requests } = await checkRequestsUsingSubWorkflow(
        subWorkflow.id
      )
      ...
      <AlertDialogContent className="max-w-md">
        <AlertDialogHeader>
          <AlertDialogTitle className="flex items-center gap-2">
            ...
            {step === 'migrate'
              ? 'Có yêu cầu đang sử dụng'
              : 'Xác nhận xóa quy trình con'}
          </AlertDialogTitle>
                ...
                <div>
                  ...
                  <div className="mt-2 p-3 bg-muted rounded-md">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Trạng thái: {subWorkflow.statusName}
                    </p>
                  </div>
              ...
              {step === 'migrate' && (
                <div>
                  <p className="text-amber-700">
                    Có <strong>{requestsInUse.length} yêu cầu</strong> đang sử
                    dụng quy trình này:
                  </p>
                  ...
                  <p className="mt-3 text-sm">
                    Bạn cần chuyển các yêu cầu này về quy trình chuẩn trước khi
                    xóa.
                  </p>
                </div>
              )}
