...
export function SubWorkflowSyncDialog({
  subWorkflow,
  open,
  onOpenChange
}: SubWorkflowSyncDialogProps) {
  ...
  useEffect(() => {
    ...
    if (standardWorkflow.version > subWorkflow.createdFromWorkflowVersion) {
      ...
      const subWorkflowStepsMap = new Map(
        subWorkflow.workflowSteps.map((step) => [step.id, step])
      )
      ...
      standardWorkflow.steps.forEach((standardStep) => {
        ...
        if (!subStep) {
          ...
          const subFieldsMap = new Map(
            subStep.fields.map((field) => [field.id, field])
          )
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const allChangeIds = changes.map(
        (change, index) =>
          `${change.type}_${change.stepId}_${change.fieldId || ''}`
      )
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      selectedChanges.forEach((changeId) => {
        ...
        const standardStep = standardWorkflow.steps.find(
          (s) => s.id === change.stepId
        )
        ...
        switch (change.type) {
          ...
          case 'new_field':
            ...
            const stepForNewField = updatedSteps.find(
              (s) => s.id === change.stepId
            )
            const standardField = standardStep.fields.find(
          ...
          case 'updated_field':
            ...
            const stepForUpdatedField = updatedSteps.find(
              (s) => s.id === change.stepId
            )
            const updatedStandardField = standardStep.fields.find(
              (f) => f.id === change.fieldId
            )
            if (stepForUpdatedField && updatedStandardField) {
              const fieldIndex = stepForUpdatedField.fields.findIndex(
                (f) => f.id === change.fieldId
              )
              if (fieldIndex !== -1) {
                stepForUpdatedField.fields[fieldIndex] = {
                  ...updatedStandardField
                }
              }
            }
            break
          ...
          case 'reorder_steps':
            ...
            const stepForReorder = updatedSteps.find(
              (s) => s.id === change.stepId
            )
            if (stepForReorder) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            So sánh và đồng bộ các thay đổi từ quy trình chuẩn (v
            {standardWorkflow?.version}) vào quy trình con "{subWorkflow.name}"
            (được tạo từ v{subWorkflow.createdFromWorkflowVersion})
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden">
          {changes.length === 0 ? (
            <Alert>
              ...
              <AlertDescription>
                Quy trình con đã được cập nhật. Không có thay đổi nào từ quy
                trình chuẩn.
              </AlertDescription>
            </Alert>
          ) : (
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">
                    Chọn tất cả ({changes.length} thay đổi)
                  </span>
                </div>
                <Badge variant="outline">
                  {selectedChanges.length} đã chọn
                </Badge>
              </div>
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <Badge variant="outline" className="text-xs">
                                  ...
                                  {change.type === 'updated_step' &&
                                    'Cập nhật bước'}
                                  {change.type === 'new_field' && 'Trường mới'}
                                  {change.type === 'updated_field' &&
                                    'Cập nhật trường'}
                                  {change.type === 'reorder_steps' &&
                                    'Thay đổi thứ tự'}
                                </Badge>
                                {change.stepName && (
                                  <Badge
        ...
        <DialogFooter>
          ...
          <Button
            onClick={handleSync}
            disabled={selectedChanges.length === 0 || isSyncing}
          >
            {isSyncing ? (
              <>
