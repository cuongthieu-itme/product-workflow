...
interface SubWorkflowContextType {
  ...
  addSubWorkflow: (subWorkflow: Omit<SubWorkflow, "id" | "createdAt" | "updatedAt">) => Promise<SubWorkflow>
  updateSubWorkflow: (id: string, subWorkflow: Partial<SubWorkflow>) => Promise<void>
  deleteSubWorkflow: (id: string) => Promise<void>
  getSubWorkflowById: (id: string) => Promise<SubWorkflow | undefined>
  getSubWorkflowByStatusId: (statusId: string) => Promise<SubWorkflow | undefined>
...
const SubWorkflowContext = createContext<SubWorkflowContextType | undefined>(undefined)
...
export function SubWorkflowProvider({ children }: { children: ReactNode }) {
  ...
  const addSubWorkflow = useCallback(
    async (subWorkflow: Omit<SubWorkflow, "id" | "createdAt" | "updatedAt">): Promise<SubWorkflow> => {
      try {
        const now = Date.now()
        ...
        const docRef = await addDoc(collection(db, "subWorkflows"), sanitizedData)
        ...
        if (subWorkflow.statusId && subWorkflow.statusId !== "") {
          try {
            ...
            console.error("Error updating product status with workflow ID:", err)
          }
        }
  ...
  const updateSubWorkflow = useCallback(
    async (id: string, subWorkflow: Partial<SubWorkflow>): Promise<void> => {
      try {
        ...
        if (process.env.NODE_ENV === "development") {
          setSubWorkflows((prev) => prev.map((item) => (item.id === id ? { ...item, ...sanitizedData } : item)))
    ...
    async (id: string): Promise<void> => {
      try {
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          toast({
            title: "Thành công (Dev Mode)",
            description: "Đã xóa quy trình con và chuyển trạng thái về quy trình chuẩn (giả lập)",
          })
        ...
        const productStatusesQuery = query(collection(db, "productStatuses"), where("workflowId", "==", id))
        ...
        const updatePromises = productStatusesSnapshot.docs.map(async (statusDoc) => {
  ...
  const getSubWorkflowByStatusId = useCallback(
    (statusId: string): Promise<SubWorkflow | undefined> => {
      return Promise.resolve(subWorkflows.find((sw) => sw.statusId === statusId))
    },
    [subWorkflows],
  )
  ...
  const getStepsByIds = useCallback(
    (stepIds: string[]) => {
      ...
      console.log("📊 standardWorkflow available:", !!standardWorkflowContext?.standardWorkflow)
      ...
      console.log(
        ...
        standardWorkflowContext.standardWorkflow.steps.map((s) => ({ id: s.id, name: s.name })),
      )

      const foundSteps = stepIds
        .map((stepId) => {
          const step = standardWorkflowContext.standardWorkflow.steps.find((step) => step.id === stepId)
          if (!step) {
  ...
  return <SubWorkflowContext.Provider value={value}>{children}</SubWorkflowContext.Provider>
}
