...
export function AddWorkflowProcessForm({
  ...
  const { addWorkflowProcess, updateWorkflowProcess, isWorkflowProcessNameExists, getWorkflowProcessByStatusId } =
    useWorkflowProcess()
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.statusId) {
      ...
      if (existingProcess && existingProcess.id !== editingProcess?.id) {
        setStatusError("Trạng thái này đã có quy trình, vui lòng chọn trạng thái khác")
        isValid = false
      }
    }

    return isValid
  }, [formData, isWorkflowProcessNameExists, getWorkflowProcessByStatusId, editingProcess])
  ...
  const handleSubmit = useCallback(
    ...
    [validateForm, formData, editingProcess, updateWorkflowProcess, addWorkflowProcess, toast, onProcessAdded, onClose],
  )
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle>{editingProcess ? "Chỉnh sửa quy trình" : "Thêm quy trình làm việc mới"}</DialogTitle>
          <DialogDescription>
            {editingProcess
