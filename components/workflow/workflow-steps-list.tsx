...
import { useWorkflowProcess, type WorkflowStep } from "./workflow-process-context"
import { PlusCircle, Edit, Trash2, ChevronRight, ChevronDown, Clock, AlertCircle, MoveUp, MoveDown } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function WorkflowStepsList({ process, onStepsUpdated }: WorkflowStepsListProps) {
  const { toast } = useToast()
  const { addWorkflowStep, updateWorkflowStep, deleteWorkflowStep, reorderWorkflowSteps } = useWorkflowProcess()
...
`
  ...
  }, [validateForm, formData, editingStep, process.id, updateWorkflowStep, addWorkflowStep, toast, onStepsUpdated])
    ...
    <div className="space-y-6">
      ...
      {process.steps.length === 0 ? (
        <Card>
          <CardContent className="pt-6 text-center">
            <p className="text-muted-foreground">Chưa có bước nào trong quy trình này.</p>
        ...
        <div ref={stepsContainerRef} className="space-y-4 custom-scrollbar">
          {process.steps.map((step, index) => (
            ...
            >
              <Card className={expandedStepId === step.id ? "border-primary" : ""}>
                <CardHeader className="p-4 pb-0">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleEditStep(step)}>
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => toggleExpandStep(step.id)}>
                    ...
                    <div className="border-t mt-2 pt-4">
                      <div className="flex border-b mb-4">
                        <button
                          className={`px-4 py-2 font-medium text-sm ${
                            activeTab === "info" ? "border-b-2 border-primary text-primary" : "text-muted-foreground"
                          }`}
                      ...
                      {activeTab === "info" && (
                        <div className="space-y-4">
                          <div>
                            <Label className="text-sm font-medium">Tên bước</Label>
                          ...
                          <div>
                            ...
                            <div className="mt-1 text-sm">{step.estimatedDays} ngày</div>
                          </div>
                          <div className="flex justify-end">
                            <Button size="sm" onClick={() => handleEditStep(step)}>
      ...
      <Dialog open={isAddStepOpen} onOpenChange={setIsAddStepOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{editingStep ? "Chỉnh sửa bước" : "Thêm bước mới"}</DialogTitle>
            <DialogDescription>
              {editingStep
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleInputChange("estimatedDays", Number.parseInt(e.target.value) || 1)}
              />
            </div>
          </div>
      ...
      <AlertDialog open={!!stepToDelete} onOpenChange={(open) => !open && setStepToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bước "{stepToDelete?.name}" sẽ bị xóa khỏi quy trình. Hành động này không thể hoàn tác.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Hủy</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDeleteStep} className="bg-destructive text-destructive-foreground">
              Xóa
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
