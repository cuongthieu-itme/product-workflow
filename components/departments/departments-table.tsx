...
export function DepartmentsTable() {
  ...
  const fetchDepartments = async () => {
    ...
    try {
      ...
      if (departmentsSnapshot.empty) {
        console.log("Không có phòng ban nào trong Firestore, tạo dữ liệu mẫu...")
        ...
        const defaultDepartments = [
          ...
          },
          {
            ...
            manager: "",
            members: [],
            accessRights: ["view_all", "edit_all", "delete", "approve", "export"],
            createdAt: serverTimestamp(),
          },
        ]
      ...
      setError(`Lỗi khi lấy danh sách phòng ban: ${error instanceof Error ? error.message : String(error)}`)
      toast({
        variant: "destructive",
        title: "Lỗi",
  ...
  const handleAccessRightChange = (rightId: string, checked: boolean) => {
    ...
    setEditingDepartment((prev) => {
      if (!prev) return prev
      const accessRights = Array.isArray(prev.accessRights) ? prev.accessRights : []

      if (checked) {
        return { ...prev, accessRights: [...accessRights, rightId] }
      } else {
        return { ...prev, accessRights: accessRights.filter((id) => id !== rightId) }
      }
    })
  }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const departmentsRef = collection(db, "departments")
      const nameQuery = query(departmentsRef, where("name", "==", editingDepartment.name.trim()))
      const nameQuerySnapshot = await getDocs(nameQuery)

      // Kiểm tra xem có phòng ban nào khác có tên trùng không
      const duplicateName = nameQuerySnapshot.docs.some((doc) => doc.id !== editingDepartment.id)

      if (duplicateName) {
        setError("Tên phòng ban đã tồn tại trong hệ thống. Vui lòng chọn tên khác.")
        toast({
          variant: "destructive",
          title: "Lỗi",
      ...
      if (editingDepartment.id) {
        ...
        const currentDeptUsers = users.filter((user) => user.department === editingDepartment.id)
        const currentDeptUserIds = currentDeptUsers.map((user) => user.id)

        // Người dùng cần thêm vào phòng ban
        const usersToAdd = selectedUsers.filter((id) => !currentDeptUserIds.includes(id))

        // Người dùng cần xóa khỏi phòng ban
        const usersToRemove = currentDeptUserIds.filter((id) => !selectedUsers.includes(id))

        // Cập nhật người dùng trong Firestore
        for (const userId of usersToAdd) {
          const userRef = doc(db, "users", userId)
          await updateDoc(userRef, { department: editingDepartment.id })
          console.log("Đã thêm người dùng", userId, "vào phòng ban", editingDepartment.id)
        }

        for (const userId of usersToRemove) {
          const userRef = doc(db, "users", userId)
          await updateDoc(userRef, { department: "" })
          console.log("Đã xóa người dùng", userId, "khỏi phòng ban", editingDepartment.id)
        }
        ...
        const updatedUsers = users.map((user) => {
          ...
          if (user.department === editingDepartment.id && !selectedUsers.includes(user.id)) {
      ...
      setError(`Lỗi khi cập nhật phòng ban: ${error instanceof Error ? error.message : String(error)}`)
      toast({
        variant: "destructive",
        title: "Lỗi",
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const usersInDepartment = users.filter((user) => user.department === deletingDepartment.id)
      ...
      const updatedDepartments = departments.filter((department) => department.id !== deletingDepartment.id)
      ...
      setError(`Lỗi khi xóa phòng ban: ${error instanceof Error ? error.message : String(error)}`)
      toast({
        variant: "destructive",
        title: "Lỗi",
        description: "Không thể xóa phòng ban. Chi tiết: " + (error instanceof Error ? error.message : String(error)),
      })
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Button variant="outline" onClick={refreshDepartmentList} disabled={isLoading}>
          {isLoading ? (
            <>
            ...
            {isLoading ? (
              ...
              departments.map((department) => (
                <TableRow key={department.id}>
                  ...
                  <TableCell>{department.manager ? getUserName(department.manager) : "Chưa có quản lý"}</TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Users className="h-4 w-4 text-muted-foreground" />
                      <span>{department.members?.length || countMembers(department.id)}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {department.accessRights && department.accessRights.length > 0 ? (
                        department.accessRights.map((rightId) => (
                          <TooltipProvider key={rightId}>
                            <Tooltip>
                              ...
                              <TooltipContent>
                                <p>{accessRights.find((r) => r.id === rightId)?.description}</p>
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                        ))
                      ) : (
                        <span className="text-xs text-muted-foreground">Không có quyền đặc biệt</span>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Dialog>
                        ...
                        {editingDepartment && (
                          <DialogContent className="sm:max-w-[600px]">
                            ...
                            <ScrollArea className="max-h-[70vh]">
                              <div className="grid gap-4 py-4 pr-4">
                                <div className="grid grid-cols-4 items-center gap-4">
                                  ...
                                  <Input id="edit-id" value={editingDepartment.id} className="col-span-3" disabled />
                                </div>
                                <div className="grid grid-cols-4 items-center gap-4">
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      setEditingDepartment({ ...editingDepartment, name: newName })
                                      checkEditDepartmentName(newName, editingDepartment.id)
                                    }}
                                ...
                                <div className="grid grid-cols-4 items-center gap-4">
                                  ...
                                  <Select
                                    ...
                                    onValueChange={(value) =>
                                      setEditingDepartment({ ...editingDepartment, manager: value })
                                    }
                                  >
                                ...
                                <div className="grid grid-cols-4 items-start gap-4">
                                  <Label className="text-right pt-2">Quyền Truy Cập</Label>
                                      ...
                                      {accessRights.map((right) => (
                                        <div key={right.id} className="flex items-start space-x-2">
                                          <Checkbox
                                            ...
                                            checked={(editingDepartment.accessRights || []).includes(right.id)}
                                ...
                                <div className="grid grid-cols-4 items-start gap-4">
                                  <Label className="text-right pt-2">Thành Viên</Label>
                            ...
                            <DialogFooter>
                              <Button type="button" onClick={handleSaveEdit} disabled={isLoading || editNameExists}>
                      ...
                      <Button variant="outline" size="sm" onClick={() => openDeleteDialog(department)}>
        ...
        <DialogContent>
          ...
          {deletingDepartment && countMembers(deletingDepartment.id) > 0 && (
            <div className="mt-2 text-red-500 text-sm">
              Lưu ý: Phòng ban này có {countMembers(deletingDepartment.id)} thành viên. Khi xóa phòng ban, các thành
              viên sẽ không còn thuộc phòng ban nào.
            </div>
          )}
          ...
          <DialogFooter>
            ...
            <Button onClick={confirmDelete} disabled={isLoading} className="bg-red-500 hover:bg-red-600 text-white">
      ...
      <Dialog open={!!viewingDepartment} onOpenChange={(open) => !open && setViewingDepartment(null)}>
        <DialogContent className="sm:max-w-[700px]">
          {viewingDepartment && (
            <>
              <DialogHeader>
                <DialogTitle>Chi tiết phòng ban: {viewingDepartment.name}</DialogTitle>
                <DialogDescription>{viewingDepartment.description}</DialogDescription>
              </DialogHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4">
                <div className="flex flex-col gap-1">
                  ...
                  <span>
                    {viewingDepartment.manager ? getUserName(viewingDepartment.manager) : "Chưa có trưởng phòng"}
                  </span>
                </div>
                <div className="flex flex-col gap-1">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span>
                    {typeof viewingDepartment.createdAt === "string"
                      ? new Date(viewingDepartment.createdAt).toLocaleDateString("vi-VN")
                      : "Không xác định"}
                  </span>
                </div>
              </div>
              ...
              <div className="py-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {viewingDepartment.accessRights && viewingDepartment.accessRights.length > 0 ? (
                      ...
                      {departmentMembers.map((user) => (
                        <div
                          ...
                          className={`flex items-center justify-between p-2 border rounded-md ${
                            user.id === viewingDepartment.manager ? "bg-primary/5 border-primary/20" : ""
                          }`}
                        >
                          <div className="flex items-center gap-2">
