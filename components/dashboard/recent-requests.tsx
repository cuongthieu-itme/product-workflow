...
const getStatusDisplay = (request: any) => {
  ...
  if (request.workflowSteps && request.workflowSteps.length > 0) {
    const completedSteps = request.workflowSteps.filter(
      (step: any) => step.status === 'completed'
    ).length
    const totalSteps = request.workflowSteps.length
    const hasSkippedStep = request.workflowSteps.some(
      (step: any) => step.status === 'skipped'
    )
    const hasInProgressStep = request.workflowSteps.some(
    ...
    if (userRole !== 'admin' && userDepartment) {
      filtered = requests.filter((req) => {
        ...
        if (userDept === 'rd' || userDept === 'r&d') {
          const hasSkippedStep = req.workflowSteps?.some(
            (step: any) => step.status === 'skipped'
          )
            ...
            filteredRequests.map((request) => {
              ...
              return (
                <TableRow key={request.id}>
                  ...
                  <TableCell>
                    {request.creator?.name || 'Không xác định'}
                  </TableCell>
                  <TableCell title={formatDateTime(request.createdAt)}>
