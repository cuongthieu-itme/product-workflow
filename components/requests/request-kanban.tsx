...
export function RequestKanban() {
  ...
  const [columnedRequests, setColumnedRequests] = useState<
    Record<string, any[]>
  >({})
  ...
  const getStepColor = (stepName: string) => {
    ...
    if (name.includes('hoàn thành') || name.includes('completed'))
      return 'bg-green-100'
    if (name.includes('từ chối') || name.includes('rejected'))
      return 'bg-red-100'
    if (name.includes('tạm dừng') || name.includes('on hold'))
      return 'bg-gray-200'
    if (name.includes('kiểm tra') || name.includes('check'))
      return 'bg-blue-100'
    if (name.includes('phê duyệt') || name.includes('approve'))
      return 'bg-yellow-100'
    if (name.includes('thực hiện') || name.includes('execute'))
    ...
    if (name.includes('template') || name.includes('mockup'))
  ...
  const handleMoveRequest = async (request: any, targetStepId: string) => {
    try {
      ...
      const targetStep = subWorkflow.workflowSteps?.find(
        (step) => step.id === targetStepId && step.isVisible
      )
  ...
  useEffect(() => {
    ...
    const allColumns: KanbanColumn[] = Array.from(stepMap.entries()).map(
  ...
  useEffect(() => {
    if (loading || !requests || !Array.isArray(requests) || !columns.length)
      return
    ...
    requests.forEach((request) => {
      ...
      console.log(
        `Request ${request.id} (status: ${request.status}) -> column: ${targetColumnId}`
      )
      ...
      if (targetColumn) {
        ...
        console.log(
          `Added request ${request.id} to column ${targetColumn.title}`
        )
      } else {
        ...
        if (requestsByColumn['no-workflow']) {
          ...
          console.log(
            `Added request ${request.id} to no-workflow column as fallback`
          )
        }
      }
    })
          ...
          {columns.map((column) => {
            ...
            return (
              <div key={column.id} className="flex-shrink-0 w-80 h-full">
                <div
                  className={cn(
                    'h-full border rounded-lg bg-white shadow-sm',
                    column.color
                  )}
                >
                  <div className="pb-3">
                        ...
                        {columnRequests.length > 0 ? (
                          columnRequests.map((request) => {
                            ...
                            const currentAssignee =
                              getCurrentStepAssignee(request)
                            return (
                              <div
                                key={request.id}
