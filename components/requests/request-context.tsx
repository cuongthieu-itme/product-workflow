...
interface RequestContextType {
  ...
  loading: boolean
  addRequest: (request: Omit<Request, "id" | "code" | "createdAt" | "updatedAt">) => Promise<string>
  updateRequest: (
    id: string,
  ...
  updateCustomerRequests: (customerId: string, requestId: string) => Promise<void>
...
export function RequestProvider({ children }: { children: React.ReactNode }) {
  ...
  const { addMaterialRequest: addMaterialRequestToMaterialContext } = materialContext || {}
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const materialImportRequestsSnapshot = await getDocs(materialImportRequestsRef)
      ...
      if (dataSourcesData.length === 0) {
        const initialDataSources = [
          ...
          { type: "other", name: "Hội chợ triển lãm", specificSource: "Triển lãm nội thất Hà Nội 2023" },
        ]
        ...
        const updatedDataSourcesData = updatedDataSourcesSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        })) as DataSource[]
  ...
  const addRequest = useCallback(
    async (request: Omit<Request, "id" | "code" | "createdAt" | "updatedAt">) => {
      try {
        const now = new Date()
        ...
        if (safeRequest.dataSource.customerId) {
          await updateCustomerRequests(safeRequest.dataSource.customerId, docRef.id)
        }
  ...
  const updateRequest = useCallback(
    async (id: string, request: Partial<Omit<Request, "id" | "code" | "createdAt" | "updatedAt">>) => {
      try {
        ...
        Object.keys(request).forEach((key) => {
          ...
          if (
            ...
            JSON.stringify(oldRequest[typedKey]) !== JSON.stringify(request[typedKey])
          ) {
        ...
        const firestoreData = convertDateToTimestamp(sanitizeFirestoreData(sanitizedData))
      ...
      try {
        ...
        if (addMaterialRequestToMaterialContext) {
          addMaterialRequestToMaterialContext({
            ...
            expectedDate:
              materialImportRequest.expectedDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
            supplier: materialImportRequest.supplier || "",
            status: "pending",
            reason: materialImportRequest.reason || "Yêu cầu từ phát triển sản phẩm",
  ...
  const updateCustomerRequests = useCallback(async (customerId: string, requestId: string) => {
  ...
  const getRequestsByCustomerId = useCallback(
    async (customerId: string) => {
      ...
      try {
        ...
        const q = query(customerRequestsRef, where("customerId", "==", customerId))
        const querySnapshot = await getDocs(q)
