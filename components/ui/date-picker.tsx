      ...
      <div className="flex">
        ...
        <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
          ...
          <PopoverContent className="w-auto p-0" align="end">
            <Calendar mode="single" selected={date} onSelect={handleCalendarSelect} initialFocus locale={vi} />
          </PopoverContent>
        </Popover>
      </div>
      {inputValue.length === 10 && !isDateValid() && (
        <p className="text-xs text-red-500 mt-1">Ngày không hợp lệ. Vui lòng kiểm tra lại.</p>
      )}
    </div>
  )
}
