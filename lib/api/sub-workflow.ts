  ...
  try {
    ...
    if (subWorkflowData.statusId) {
      ...
      if (statusDoc.exists()) {
        ...
        if (statusData.workflowId && statusData.workflowId !== "standard-workflow") {
          console.log(`Status ${subWorkflowData.statusId} already has workflowId: ${statusData.workflowId}`)
          return {
            success: false,
      ...
      const q = query(collection(db, "subWorkflows"), where("statusId", "==", subWorkflowData.statusId))
      const querySnapshot = await getDocs(q)

      if (!querySnapshot.empty) {
        const existingWorkflow = querySnapshot.docs[0].data()
        console.log(`Found existing sub-workflow for status ${subWorkflowData.statusId}:`, existingWorkflow)
        return {
          success: false,
    ...
    return {
      success: false,
      message: "Lỗi khi tạo quy trình con: " + (error instanceof Error ? error.message : String(error)),
    }
  }
}
        ...
        if (currentData.statusId !== updates.statusId) {
          ...
          if (statusDoc.exists()) {
            ...
            if (statusData.workflowId) {
              console.log(`Status ${updates.statusId} already has workflowId: ${statusData.workflowId}`)
...
export const getSubWorkflowsByWorkflowProcessId = async (workflowProcessId: string) => {
  try {
    const q = query(collection(db, "subWorkflows"), where("parentWorkflowId", "==", workflowProcessId))
    const querySnapshot = await getDocs(q)
...
// Cập nhật trạng thái với ID quy trình con
export const updateStatusWithWorkflow = async (statusId: string, workflowId: string) => {
  try {
...
export const checkStatusHasWorkflow = async (statusId: string) => {
  try {
    ...
    if (statusDoc.exists()) {
      ...
      // Kiểm tra xem trạng thái có workflowId không
      if (statusData.workflowId) {
        console.log(`Status ${statusId} has workflowId: ${statusData.workflowId}`)
        return true
      }
    }
...
export const checkStatusHasSubWorkflow = async (statusId: string) => {
  try {
    ...
    const q = query(collection(db, "subWorkflows"), where("statusId", "==", statusId))
    const querySnapshot = await getDocs(q)
    const hasWorkflow = !querySnapshot.empty
    console.log("Status has workflow:", hasWorkflow, "Number of docs:", querySnapshot.size)
    return hasWorkflow
  } catch (error) {
